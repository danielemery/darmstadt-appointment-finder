...
- Signals every run to healthchecks.io: `/start` when the run begins (so healthchecks records run durations), a heartbeat on success, the `/fail` endpoint (with error details) when any check or notification failed.
